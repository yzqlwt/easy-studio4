...
import { QKID_GATE } from '../configs/config';
import { SYMBOL_FOR_FETCH_GATE_API } from '../constants/fetchType';
...
export default function thunkMiddleware() {
  return (next) => (action) => {
    const API_OPTIONS = action[SYMBOL_FOR_FETCH_GATE_API];
    console.log(action);
    const fetchOptions = {};
    let queryString = '';
    ...
    return fetch(QKID_GATE + uri + queryString, fetchOptions)
      .then((response) =>
